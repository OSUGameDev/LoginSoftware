...
namespace GameDevLogin
{
    static class MagReaderHelper
    {
        ...
        /// </summary>
        public static bool ReadingCards = true;

        /// <summary>
        /// Event to be called when an enter key is pressed
        /// </summary>
        public static event Action<string> onEnter;
        
        /// <summary>
        /// Current captured keyboard input
        /// </summary>
        private static string CurrentLine;

        /// <summary>
        /// Action to be called whenever a keyboard key is pressed
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        static private void KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                onEnter?.Invoke(CurrentLine+'\n');
                CurrentLine = "";
            }
            else
            {
                char item = KeyToChar(e.Key);
                if (item != '\x00')
                    CurrentLine += item;
            }
        }
        ...
        /// <returns>character equivelent</returns>
